<?php
function aromatic_info2_setting( $wp_customize ) {
	 ...
	 */
		...
		$wp_customize->add_setting( 'info2_contents', 
			array(
			 ...
			 'priority' => 8,
			 'default' => aromatic_get_info2_default()
			)
		);
...
add_action( 'customize_register', 'aromatic_info2_setting' );

//selective refresh
function aromatic_info2_section_partials( $wp_customize ){	
	
	// info content
	$wp_customize->selective_refresh->add_partial( 'info2_contents', array(
		'selector'            => '.info-home2 .info-service-flex'
	) );
	
}
add_action( 'customize_register', 'aromatic_info2_section_partials' );